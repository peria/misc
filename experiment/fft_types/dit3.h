...
class DIT3 : public FMT {
 public:
  DIT3(int logn) : FMT(logn) { Init(); }
  double GetMemory() const override;

  static const char* name() { return "DIT3"; }
...
void DIT3::Init() {
  {
    ...
    ws_.push_back(Complex(std::cos(theta * 2), std::sin(theta * 2)));
    ws_.push_back(Complex(std::cos(theta * 3), std::sin(theta * 3)));
    for (int i = 1; i < logn_ - 2; ++i) {
      int m = 1 << i;
      theta *= 0.5;
      Complex w1(std::cos(theta), std::sin(theta));
      Complex w2(std::cos(theta * 2), std::sin(theta * 2));
      Complex w3(std::cos(theta * 3), std::sin(theta * 3));
      for (int j = 0; j < m; ++j) {
        ws_.push_back(w1 * ws_[3 * j]);
        ws_.push_back(w2 * ws_[3 * j + 1]);
        ws_.push_back(w3 * ws_[3 * j + 2]);
      }
    }
  }
...
void DIT3::IRft(Complex* x) const {
...
void DIT3::IDft(Complex* x) const {
  int m = 1;
  int l = n_;
  for (int i = 0; i < log4n_; ++i) {
    l /= 4;
...
void DIT3::Dft4(Complex* x,
                const int m,
                const int l,
                const int j0,
                const int jn) const {
  for (int j = j0; j < jn; ++j) {
    const Complex& w1 = ws_[3 * j];
    const Complex w2 = ws_[3 * j + 1];
    const Complex w3 = ws_[3 * j + 2];
    for (int k = 0; k < m; ++k) {
      int k0 = 4 * j * m + k;
      int k1 = 4 * j * m + m + k;
      int k2 = 4 * j * m + 2 * m + k;
...
void DIT3::IDft4(Complex* x,
                 const int m,
                 const int l,
                 const int j0,
                 const int jn) const {
  for (int j = j0; j < jn; ++j) {
    const Complex w1 = ws_[3 * j].conj();
    const Complex w2 = ws_[3 * j + 1].conj();
    const Complex w3 = ws_[3 * j + 2].conj();
    for (int k = 0; k < m; ++k) {
      int k0 = 4 * j * m + k;
      int k1 = 4 * j * m + m + k;
      int k2 = 4 * j * m + 2 * m + k;
